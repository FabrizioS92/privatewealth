...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const NAV_ITEMS = [
    ...
    { to: "/portfolio", label: t("nav_portfolio"), icon: PieChart },
    { to: "/transactions", label: t("nav_transactions"), icon: ArrowLeftRight },
    { to: "/dividends", label: t("nav_dividends"), icon: Coins },
    { to: "/import", label: t("nav_import"), icon: Upload },
  ] as const;
    ...
    <div className="min-h-screen bg-background">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-border bg-sidebar/90 backdrop-blur-xl md:flex">
        <div className="flex h-20 items-center gap-3 px-6">
          <FolioMark size={40} />
          <div>
            <p className="font-display text-xl font-semibold leading-none tracking-tight">Folio</p>
            <p className="mt-1 text-[10px] font-medium tracking-wide text-muted-foreground">
              {t("nav_portfolio_tracker")}
            </p>
          </div>
        </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? "bg-primary text-primary-foreground shadow-[var(--shadow-mint)]" : "text-muted-foreground",
              )}
            >
