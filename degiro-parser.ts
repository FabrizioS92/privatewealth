...
async function parseTransactionRow(row: Record<string, string>): Promise<ParsedTransaction | null> {
  ...
  // FIX: "Valore locale" contiene un numero, non un codice valuta — rimosso dal fallback.
  // "Valuta locale" è la colonna corretta per il codice valuta nel CSV DEGIRO IT.
  const currency =
    (
      pick(row, ["Mutatie", "Currency", "Valuta", "Divisa", "Valuta locale"]) ||
      "EUR"
    )
      .trim()
      .toUpperCase()
  ...
  // FIX: il CSV DEGIRO riporta il totale con segno negativo per i BUY (uscita di cassa).
  // Usiamo sempre Math.abs per normalizzare, poi ricostruiamo con la formula se assente.
  const rawTotal = normalizeNumber(totalRaw);
